...
SANTILLANA_CONNECT_LOGIN_URL = "https://identity.santillanaconnect.com/login/"
SANTILLANA_CONNECT_ENTRY_URL = (
    "https://identity.santillanaconnect.com/connect/postlogout"
    "?redirect_uri=https%3A%2F%2Fedi-compartir-pe.stn-neds.com%2Fts%2Fview%2Faccess"
    "&client_id=neds_client_compartir_peru_pro"
)
SANTILLANA_CONNECT_ACCESS_URL = "https://edi-compartir-pe.stn-neds.com/ts/view/access"
...
def _resolve_login_password_columns(df: pd.DataFrame) -> Tuple[Optional[str], Optional[str]]:
    columns = [str(col) for col in df.columns]
    normalized = {col: _normalize_header_key(col) for col in columns}
    ...
    login_col = next(
        (col for col in columns if normalized.get(col, "") in login_aliases),
        None,
    )
    password_col = next(
        (col for col in columns if normalized.get(col, "") in password_aliases),
        None,
    )

    if (not login_col or not password_col) and len(columns) >= 8:
        # Fallback pedido: columna G y H.
        login_col = login_col or columns[6]
        password_col = password_col or columns[7]

    return login_col, password_col


def _load_connect_credentials_from_excel(
    excel_bytes: bytes,
    ...
    with pd.ExcelFile(BytesIO(excel_bytes), engine="openpyxl") as excel:
        if not excel.sheet_names:
            raise RuntimeError("El archivo no contiene hojas.")
        resolved_sheet = sheet_name.strip() if isinstance(sheet_name, str) else ""
        if not resolved_sheet:
            resolved_sheet = str(excel.sheet_names[0])
        elif resolved_sheet not in excel.sheet_names:
            raise RuntimeError(
                f"No existe la hoja '{resolved_sheet}'. Hojas: {', '.join(excel.sheet_names)}"
            )
        df = pd.read_excel(excel, sheet_name=resolved_sheet, dtype=str)

    if df.empty:
        raise RuntimeError("No hay filas para procesar en el Excel.")

    login_col, password_col = _resolve_login_password_columns(df)
    if not login_col or not password_col:
        raise RuntimeError(
            "No se encontraron columnas Login/Password ni columnas G/H."
        )

    rows: List[Dict[str, object]] = []
    for idx, row in df.iterrows():
        login = str(row.get(login_col) or "").strip()
        password = str(row.get(password_col) or "").strip()
    ...
    if not rows:
        raise RuntimeError("No se encontraron credenciales validas en Login/Password.")
    return rows
...
def _has_login_form(html_lower: str) -> bool:
    if not html_lower:
        return False
    has_user = 'name="username"' in html_lower or "id_username" in html_lower
    has_pass = 'name="password"' in html_lower or "id_password" in html_lower
    return has_user and has_pass


def _verify_connect_login(
    username: str,
    password: str,
    timeout: int = 20,
) -> Dict[str, object]:
    ...
    if not username_txt or not password_txt:
        return {
            "ok": False,
            "detalle": "Login o password vacio.",
            "http_get": None,
            "http_post": None,
            "url_final": "",
    ...
    login_post_url = str(res_get.url or SANTILLANA_CONNECT_LOGIN_URL).strip()
    if not login_post_url:
        login_post_url = SANTILLANA_CONNECT_LOGIN_URL

    csrf_token = _extract_csrf_token(str(res_get.text or ""))
    if not csrf_token:
        try:
            # Fallback directo a /login por si el flujo de entry cambia.
            res_get_login = session.get(
                SANTILLANA_CONNECT_LOGIN_URL,
                timeout=timeout,
                allow_redirects=True,
            )
            token_fallback = _extract_csrf_token(str(res_get_login.text or ""))
            if token_fallback:
                csrf_token = token_fallback
                login_post_url = str(res_get_login.url or SANTILLANA_CONNECT_LOGIN_URL).strip()
        except requests.RequestException:
            pass
    if not csrf_token:
        return {
            "ok": False,
            "detalle": "No se encontro csrfmiddlewaretoken.",
            "http_get": int(res_get.status_code),
            "http_post": None,
            "url_final": str(res_get.url or ""),
            "ms": int((time.perf_counter() - started_at) * 1000),
        }
    ...
    headers_post = {
        ...
        "Origin": "https://identity.santillanaconnect.com",
    }
    try:
        ...
        return {
            "ok": False,
            "detalle": f"Error POST login: {exc}",
            "http_get": int(res_get.status_code),
            "http_post": None,
            "url_final": str(res_get.url or ""),
            "ms": int((time.perf_counter() - started_at) * 1000),
    ...
    final_url = str(res_post.url or "")
    final_url_norm = final_url.lower().rstrip("/")
    html_lower = str(res_post.text or "").lower()
    login_form_present = _has_login_form(html_lower)
    auth_error_markers = [
        "credenciales",
        "usuario o contrase",
        "invalid",
    ...
    has_auth_error = any(marker in html_lower for marker in auth_error_markers)

    probe_status = None
    probe_location = ""
    logged_by_probe_login = False
    try:
        probe_login = session.get(
            SANTILLANA_CONNECT_LOGIN_URL,
            timeout=timeout,
            allow_redirects=False,
        )
        probe_status = int(probe_login.status_code)
        probe_location = str(probe_login.headers.get("Location") or "").strip()
        probe_location_norm = probe_location.lower()
        if probe_status in (301, 302, 303, 307, 308) and "/login" not in probe_location_norm:
            logged_by_probe_login = True
    except requests.RequestException:
        pass

    probe_access_status = None
    probe_access_location = ""
    logged_by_probe_access = False
    try:
        probe_access = session.get(
            SANTILLANA_CONNECT_ACCESS_URL,
            timeout=timeout,
            allow_redirects=False,
        )
        probe_access_status = int(probe_access.status_code)
        probe_access_location = str(probe_access.headers.get("Location") or "").strip()
        probe_access_location_norm = probe_access_location.lower()
        if probe_access_status in (301, 302, 303, 307, 308):
            if "/login" not in probe_access_location_norm:
                logged_by_probe_access = True
        elif 200 <= probe_access_status < 300:
            probe_html = str(probe_access.text or "").lower()
            if not _has_login_form(probe_html):
                logged_by_probe_access = True
    except requests.RequestException:
        pass

    redirected_out_login = bool(final_url_norm) and "/login" not in final_url_norm
    target_norm = SANTILLANA_CONNECT_ACCESS_URL.lower().rstrip("/")
    reached_target = bool(final_url_norm) and target_norm in final_url_norm
    ok = (
        reached_target
        or logged_by_probe_access
        or logged_by_probe_login
        or (redirected_out_login and not login_form_present and not has_auth_error)
    )
    detalle = "Acceso correcto." if ok else "Credenciales invalidas o sesion no iniciada."
    if not ok and has_auth_error:
    ...
    return {
        ...
        "detalle": detalle,
        "http_get": int(res_get.status_code),
        "http_post": int(res_post.status_code),
        "url_final": final_url,
        "probe_status": probe_access_status if probe_access_status is not None else probe_status,
        "probe_location": (
            probe_access_location if probe_access_location else probe_location
        ),
        "probe_login_status": probe_status,
        "probe_login_location": probe_location,
        "probe_access_status": probe_access_status,
        "probe_access_location": probe_access_location,
        "ms": int((time.perf_counter() - started_at) * 1000),
    }
...
with tab_crud_alumnos:
    ...
    with st.container(border=True):
        st.markdown("**4) Verificacion acceso Santillana Connect**")
        st.caption(
            "Valida acceso masivo desde Excel. Detecta columnas Login/Password o fallback G/H."
        )
        col_login_upl, col_login_cfg = st.columns([1.8, 1.2], gap="large")
        with col_login_upl:
            uploaded_login_excel = st.file_uploader(
                "Excel credenciales",
                type=["xlsx"],
                key="connect_login_excel_uploader",
                help="Debe incluir Login y Password. Si no hay encabezados, usa columnas G/H.",
            )
            connect_sheet_name = st.text_input(
                "Hoja (opcional)",
                key="connect_login_sheet_name",
                placeholder="Dejar vacio para primera hoja",
            )
        with col_login_cfg:
            max_accounts = st.number_input(
                "Maximo cuentas a validar",
                min_value=1,
                max_value=500,
                value=20,
                step=1,
                key="connect_login_max_accounts",
            )
            workers = st.slider(
                "Paralelismo",
                min_value=1,
                ...
                value=6,
                step=1,
                key="connect_login_workers",
            )
            timeout_login = st.number_input(
                "Timeout por cuenta (s)",
                min_value=5,
                max_value=120,
                value=25,
                step=1,
                key="connect_login_timeout",
            )

        run_connect_check = st.button(
            "Verificar accesos",
            type="primary",
            key="connect_login_run",
        )

        if run_connect_check:
            if not uploaded_login_excel:
                st.error("Sube un Excel con Login/Password.")
                st.stop()
            try:
                credentials = _load_connect_credentials_from_excel(
                    excel_bytes=uploaded_login_excel.read(),
                    sheet_name=str(connect_sheet_name or "").strip() or None,
                )
            except Exception as exc:  # pragma: no cover - UI
                st.error(f"Error leyendo Excel: {exc}")
                st.stop()

            total_raw = len(credentials)
            limit = int(max_accounts)
            if limit > 0 and total_raw > limit:
                credentials = credentials[:limit]
            total = len(credentials)
            if total <= 0:
                st.error("No se encontraron cuentas para verificar.")
                st.stop()
            ...
            with ThreadPoolExecutor(max_workers=int(workers)) as executor:
                futures = {
                    executor.submit(
                        _verify_connect_login,
                        str(entry.get("login") or ""),
                ...
                for future in as_completed(futures):
                    ...
                    completed += 1
                    login_txt = str(entry.get("login") or "").strip()
                    status.write(
                        f"Verificando {completed}/{total}: {login_txt or '(sin login)'}"
                    )
                    ...
                    resultados.append(
                        {
                            "Fila Excel": int(entry.get("fila_excel") or 0),
                            "Login": login_txt,
                            "Resultado": "OK" if bool(result.get("ok")) else "Error",
                            "Detalle": str(result.get("detalle") or ""),
                            "HTTP GET": result.get("http_get"),
                            "HTTP POST": result.get("http_post"),
                            "Probe HTTP": result.get("probe_status"),
                            "Probe Location": str(result.get("probe_location") or ""),
                            "URL Final": str(result.get("url_final") or ""),
                            "Tiempo ms": result.get("ms"),
                        }
                    )
            ...
            resultados = sorted(
                resultados,
                key=lambda row: (
                    int(row.get("Fila Excel") or 0),
                    str(row.get("Login") or "").lower(),
                ),
            )

            ok_count = sum(1 for row in resultados if str(row.get("Resultado")) == "OK")
            err_count = len(resultados) - ok_count
            st.success(
                "Validacion completada. OK: {ok} | Error: {err} | Total procesadas: {total}."
                .format(ok=ok_count, err=err_count, total=len(resultados))
            )
            if total_raw > total:
                st.caption(
                    f"Se procesaron {total} de {total_raw} cuentas por el limite configurado."
                )

            st.session_state["connect_login_results"] = resultados
            st.session_state["connect_login_results_xlsx"] = _export_simple_excel(
                resultados,
                sheet_name="verificacion_login",
            )

        connect_results = st.session_state.get("connect_login_results") or []
        if connect_results:
            st.markdown("**Vista rapida (primeras 20 cuentas)**")
            preview_rows = connect_results[:20]
            card_cols = st.columns(5, gap="small")
            for idx, row in enumerate(preview_rows):
                with card_cols[idx % 5]:
                    with st.container(border=True):
                        st.markdown(f"**{row.get('Login') or '(sin login)'}**")
                        st.caption(f"Fila {row.get('Fila Excel')}")
                        if str(row.get("Resultado")) == "OK":
                            st.success("OK")
                        else:
                            st.error("Error")
                        st.caption(str(row.get("Detalle") or ""))

            _show_dataframe(connect_results, use_container_width=True)

        if st.session_state.get("connect_login_results_xlsx"):
            st.download_button(
                label="Descargar resultado verificacion",
                data=st.session_state["connect_login_results_xlsx"],
                file_name="verificacion_accesos_connect.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                key="connect_login_download",
            )
