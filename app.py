...
def _santillana_inclusiva_course_name(item: Dict[str, object]) -> str:
    fallback = ""
    for key in ("geClaseClave", "geClase", "clase_nombre", "clase"):
        value = str(item.get(key) or "").strip()
        if not value:
            continue
        if not fallback:
            fallback = value
        if _normalize_plain_text(value).startswith(
            SANTILLANA_INCLUSIVA_COURSE_PREFIX
        ):
            return value
    return fallback


def _is_santillana_inclusiva_named_class(item: Dict[str, object]) -> bool:
    for key in ("geClaseClave", "geClase", "clase_nombre", "clase"):
        value = str(item.get(key) or "").strip()
        normalized_value = re.sub(r"\s+", " ", _normalize_plain_text(value)).strip()
        if value and normalized_value.startswith(SANTILLANA_INCLUSIVA_COURSE_PREFIX):
            return True
    return False


def _build_santillana_inclusiva_removal_class_row(
    item: Dict[str, object],
    colegio_label: object,
    colegio_id: int,
) -> Optional[Dict[str, object]]:
    if not _is_santillana_inclusiva_named_class(item):
        return None
    meta = _extract_clase_base_meta(item) or {}
    ...
    curso = _santillana_inclusiva_course_name(item)
    colegio_nombre = _colegio_label_from_clase(item, fallback=colegio_label)
    return {
        "_key": f"{int(colegio_id)}:{int(clase_id)}",
        "_clase_id": int(clase_id),
        "_colegio_id": int(colegio_id),
        "Colegio": colegio_nombre or f"Colegio {int(colegio_id)}",
        "Colegio ID": int(colegio_id),
        "Clase ID": int(clase_id),
        "Nivel": str(meta.get("nivel_nombre") or "").strip(),
        "Grado": grado_nombre,
        "Nombre curso": curso or f"Clase {int(clase_id)}",
        "Alumnos": [],
        "Profesores": [],
        "Errores": [],
...
def _santillana_inclusiva_removal_visible_class_rows(
    ...
    for row in rows:
        profesores = [
            item for item in (row.get("Profesores") or []) if isinstance(item, dict)
        ]
        errors = [
            str(item).strip()
            for item in (row.get("Errores") or [])
            if str(item).strip()
        ]
        visible_rows.append(
            {
                ...
                "Grado": row.get("Grado", ""),
                "Nombre curso": row.get("Nombre curso", ""),
                "Alumnos a retirar": len(row.get("Alumnos") or []),
                "Profesores conservados": _format_profesores_asignados(profesores),
                "Observaciones": " | ".join(errors),
            }
        )
    return visible_rows


def _santillana_inclusiva_removal_student_rows(
    rows: Sequence[Dict[str, object]],
) -> List[Dict[str, object]]:
    student_rows: List[Dict[str, object]] = []
    for row in rows:
        for alumno in row.get("Alumnos") or []:
            ...
            student_rows.append(
                {
                    ...
                    "Clase ID": row.get("Clase ID", ""),
                    "Nombre curso": row.get("Nombre curso", ""),
                    "Alumno ID": alumno.get("alumno_id", ""),
                    "Alumno": alumno.get("nombre_completo", ""),
                    "Login": alumno.get("login", ""),
                    "DNI": alumno.get("dni", ""),
                }
            )
    return student_rows
...
def _clear_santillana_inclusiva_students_assignment_state() -> None:
    for state_key in list(st.session_state.keys()):
        if str(state_key).startswith("clases_inclusiva_alumnos_"):
            st.session_state.pop(state_key, None)


def _santillana_inclusiva_assignment_class_row(
    item: Dict[str, object],
    colegio_label: object,
    colegio_id: int,
) -> Optional[Dict[str, object]]:
    ...
    return {
        "colegio_id": int(colegio_id),
        "colegio": colegio_nombre or str(colegio_label or "").strip(),
        "clase_id": int(clase_id),
        "clase": _santillana_inclusiva_course_name(item)
        or f"Clase {int(clase_id)}",
        "nivel_id": int(nivel_id),
        "nivel": str(meta.get("nivel_nombre") or "").strip(),
        "grado_id": int(grado_id),
        "grado": grado_nombre,
        "grupo_id": _safe_int(meta.get("grupo_id_actual")),
        "seccion": str(meta.get("grupo_clave_actual") or "").strip(),
        "activo": bool(item.get("activo", True)),
    }


def _resolve_santillana_inclusiva_assignment_context(
    class_row: Dict[str, object],
    contexts: Sequence[Dict[str, object]],
) -> Tuple[Optional[Dict[str, object]], str]:
    nivel_id = _safe_int(class_row.get("nivel_id"))
    grado_id = _safe_int(class_row.get("grado_id"))
    if nivel_id is None or grado_id is None:
        return None, "Clase sin nivel/grado."

    grupo_id = _safe_int(class_row.get("grupo_id"))
    if grupo_id is not None:
        for context in contexts:
            if (
                _safe_int(context.get("nivel_id")) == int(nivel_id)
                and _safe_int(context.get("grado_id")) == int(grado_id)
                and _safe_int(context.get("grupo_id")) == int(grupo_id)
            ):
                return dict(context), ""
        return {
            "nivel_id": int(nivel_id),
            ...
            "nivel": str(class_row.get("nivel") or "").strip(),
            "grado": str(class_row.get("grado") or "").strip(),
            "seccion": str(class_row.get("seccion") or "").strip(),
            "seccion_norm": _normalize_seccion_key(class_row.get("seccion")),
        }, ""

    seccion_key = _normalize_seccion_key(class_row.get("seccion"))
    if not seccion_key:
        return None, "Clase sin grupoId ni seccion."

    matches = [
        dict(context)
        for context in contexts
        if _safe_int(context.get("nivel_id")) == int(nivel_id)
        and _safe_int(context.get("grado_id")) == int(grado_id)
        and _normalize_seccion_key(
    ...
    if len(matches) == 1:
        return matches[0], ""
    if not matches:
        return None, f"No se encontro grupo para seccion {seccion_key}."
    return None, f"Seccion {seccion_key} ambigua en el censo."


def _sort_santillana_inclusiva_students(
    rows: Sequence[Dict[str, object]],
) -> List[Dict[str, object]]:
    return sorted(
        ...
        key=lambda row: (
            str(row.get("apellido_paterno") or "").upper(),
            str(row.get("apellido_materno") or "").upper(),
            str(row.get("nombre") or row.get("nombre_completo") or "").upper(),
            int(_safe_int(row.get("alumno_id")) or 0),
        ),
    )


def _student_preview_names(
    rows: Sequence[Dict[str, object]],
    limit: int = 5,
) -> str:
    values: List[str] = []
    for row in rows[: max(0, int(limit))]:
        label = str(row.get("nombre_completo") or "").strip()
        if not label:
            label = " ".join(
                part
                for part in (
    ...
    if pending > 0:
        values.append(f"... y {pending} mas")
    return "; ".join(values)


def _santillana_inclusiva_assignment_visible_rows(
    rows: Sequence[Dict[str, object]],
) -> List[Dict[str, object]]:
    visible_rows: List[Dict[str, object]] = []
    for row in rows:
        ...
        visible_rows.append(
            {
                "Colegio": row.get("colegio", ""),
                "Colegio ID": row.get("colegio_id", ""),
                "Clase ID": row.get("clase_id", ""),
                "Clase": row.get("clase", ""),
                "Nivel": row.get("nivel", ""),
                "Grado": row.get("grado", ""),
                "Seccion": row.get("seccion", ""),
                "Alumnos del grado/seccion": row.get("target_count", 0),
                "Ya asignados": row.get("assigned_count", 0),
                "Faltan asignar": row.get("missing_count", 0),
                "Primeros alumnos faltantes": _student_preview_names(
                    missing_students
                ),
            }
        )
...
def _build_santillana_inclusiva_students_assignment_plan(
    token: str,
    colegio_id: int,
    colegio_label: str,
    empresa_id: int,
    ...
    summary = {
        "colegio_id": int(colegio_id),
        "colegio": str(colegio_label or "").strip(),
        "clases_inclusivas": 0,
    ...
    class_rows = [
        row
        for row in (
            _santillana_inclusiva_assignment_class_row(
                item,
    ...
    unique_class_rows: List[Dict[str, object]] = []
    for row in class_rows:
        clase_id = _safe_int(row.get("clase_id"))
        if clase_id is None or int(clase_id) in seen_class_ids:
            continue
        seen_class_ids.add(int(clase_id))
        unique_class_rows.append(row)
    unique_class_rows.sort(
        key=lambda row: (
            str(row.get("colegio") or "").upper(),
            _participantes_grado_sort_rank(row.get("grado")),
            _grupo_sort_key(
    ...
    summary["clases_inclusivas"] = len(unique_class_rows)
    if not unique_class_rows:
        return {"rows": [], "all_rows": [], "summary": summary, "errors": errors}

    _progress(0, len(unique_class_rows), "Listando secciones del colegio...")
    ...
    contexts = _build_contexts_for_nivel_grado(niveles_data)
    active_students_by_context: Dict[
        Tuple[int, int, int],
        Dict[int, Dict[str, object]],
    ] = {}

    for idx, class_row in enumerate(unique_class_rows, start=1):
        clase_id = int(class_row.get("clase_id") or 0)
        clase_label = str(class_row.get("clase") or f"Clase {clase_id}")
        _progress(
            idx,
            len(unique_class_rows),
            f"Revisando {idx}/{len(unique_class_rows)}: {clase_label}",
        )
        context, context_error = _resolve_santillana_inclusiva_assignment_context(
            class_row,
            contexts,
        )
        if context is None:
            errors.append(f"{clase_label}: {context_error}")
            continue

        nivel_id = _safe_int(context.get("nivel_id"))
        grado_id = _safe_int(context.get("grado_id"))
        grupo_id = _safe_int(context.get("grupo_id"))
        if nivel_id is None or grado_id is None or grupo_id is None:
            errors.append(f"{clase_label}: contexto incompleto.")
            continue

        context_key = (int(nivel_id), int(grado_id), int(grupo_id))
        if context_key not in active_students_by_context:
            try:
                alumnos_ctx = _fetch_alumnos_censo(
                    token=token,
                    colegio_id=int(colegio_id),
                ...
                errors.append(f"{clase_label}: alumnos del grupo: {exc}")
                continue
            active_students: Dict[int, Dict[str, object]] = {}
            for item in alumnos_ctx:
                if not isinstance(item, dict):
                    continue
                if not _censo_alumno_matches_context(item=item, context=context):
                    continue
                ...
                if alumno_id is None:
                    continue
                active_students[int(alumno_id)] = flat
            active_students_by_context[context_key] = active_students
        ...
        try:
            ...
            errors.append(f"{clase_label}: alumnos de la clase: {exc}")
            continue

        target_students = active_students_by_context.get(context_key) or {}
        current_ids = _extract_alumno_ids_from_clase_data(clase_data)
        missing_ids = sorted(set(target_students) - set(current_ids))
        missing_students = _sort_santillana_inclusiva_students(
            [
                target_students[int(alumno_id)]
                for alumno_id in missing_ids
                if int(alumno_id) in target_students
            ]
        )

        resolved_seccion = str(
            context.get("seccion_norm") or context.get("seccion") or ""
        ).strip()
        plan_row = {
            **class_row,
            "grupo_id": int(grupo_id),
            "seccion": resolved_seccion
            or str(class_row.get("seccion") or "").strip(),
            "target_count": len(target_students),
            "assigned_count": len(current_ids & set(target_students)),
            "current_count": len(current_ids),
            "missing_count": len(missing_students),
            "missing_students": missing_students,
        }
        all_rows.append(plan_row)
        summary["alumnos_objetivo"] += len(target_students)
        summary["alumnos_actuales"] += len(current_ids & set(target_students))
        summary["alumnos_faltantes"] += len(missing_students)
        if missing_students:
            plan_rows.append(plan_row)

    summary["clases_con_pendientes"] = len(plan_rows)
    summary["clases_completas"] = max(0, len(all_rows) - len(plan_rows))
    summary["errores"] = len(errors)
    return {
        "rows": plan_rows,
        "all_rows": all_rows,
        "summary": summary,
...
def _apply_santillana_inclusiva_students_assignment(
    token: str,
    rows: Sequence[Dict[str, object]],
    empresa_id: int,
    ciclo_id: int,
    ...
    total_students = sum(
        len([item for item in (row.get("missing_students") or []) if isinstance(item, dict)])
        for row in selected_rows
    )
    summary = {
        "clases_total": len(selected_rows),
        "alumnos_pendientes": int(total_students),
        "asignados_ok": 0,
        "asignados_error": 0,
        "clases_ok": 0,
        "clases_error": 0,
        "clases_sin_cambios": 0,
    }
    result_rows: List[Dict[str, object]] = []
    processed_students = 0
    for class_index, row in enumerate(selected_rows, start=1):
        clase_id = _safe_int(row.get("clase_id"))
        missing_students = [
            item
            for item in (row.get("missing_students") or [])
            if isinstance(item, dict) and _safe_int(item.get("alumno_id")) is not None
        ]
        if clase_id is None:
            ...
            result_rows.append(
                {
                    "Clase ID": "",
                    "Clase": row.get("clase", ""),
                    "Faltantes": len(missing_students),
                    "Asignados": 0,
                    "Errores": len(missing_students),
                    "Resultado": "Error",
                    "Detalle": "Clase sin ID.",
                    "_failed_students": missing_students,
                }
            )
            continue
        if not missing_students:
            summary["clases_sin_cambios"] += 1
            result_rows.append(
                {
                    "Clase ID": int(clase_id),
                    "Clase": row.get("clase", ""),
                    ...
                    "Detalle": "No habia alumnos pendientes.",
                    "_failed_students": [],
                }
            )
            continue
        ...
        failed_students: List[Dict[str, object]] = []
        for student in missing_students:
            alumno_id = _safe_int(student.get("alumno_id"))
            if alumno_id is None:
                continue
            ...
            _progress(
                ...
                "Clase {class_index}/{classes}: asignando alumno {current}/{total}".format(
                    class_index=class_index,
                    classes=max(len(selected_rows), 1),
                    current=processed_students,
                    total=max(total_students, 1),
                ),
            )
            ...
            if ok_assign:
                assigned_ok += 1
                summary["asignados_ok"] += 1
            else:
                summary["asignados_error"] += 1
                failed_students.append(student)
                assign_errors.append(f"{int(alumno_id)}: {msg_assign}")

        if assign_errors:
            summary["clases_error"] += 1
            result_status = "Parcial" if assigned_ok else "Error"
            detail = " | ".join(assign_errors[:5])
            if len(assign_errors) > 5:
                detail += f" | ... y {len(assign_errors) - 5} errores mas"
        else:
            summary["clases_ok"] += 1
            result_status = "OK"
            detail = "Asignacion aplicada."
        ...
        result_rows.append(
            {
                ...
                "Seccion": row.get("seccion", ""),
                "Faltantes": len(missing_students),
                "Asignados": int(assigned_ok),
                "Errores": len(assign_errors),
                "Resultado": result_status,
                "Detalle": detail,
                "_failed_students": failed_students,
            }
        )
...
def _update_santillana_inclusiva_assignment_cached_rows(
    result_rows: Sequence[Dict[str, object]],
) -> None:
    cached_rows = [
        row
        for row in (
            st.session_state.get("clases_inclusiva_alumnos_rows") or []
        )
        if isinstance(row, dict)
    ]
    failed_by_class: Dict[int, List[Dict[str, object]]] = {}
    applied_class_ids: Set[int] = set()
    for result in result_rows:
        if not isinstance(result, dict):
            continue
        clase_id = _safe_int(result.get("Clase ID"))
        if clase_id is None:
            continue
        applied_class_ids.add(int(clase_id))
        failed_by_class[int(clase_id)] = [
            item
            for item in (result.get("_failed_students") or [])
            if isinstance(item, dict)
        ]
    ...
    for row in cached_rows:
        clase_id = _safe_int(row.get("clase_id"))
        if clase_id is None or int(clase_id) not in applied_class_ids:
            updated_rows.append(row)
            continue
        failed_students = failed_by_class.get(int(clase_id), [])
        if not failed_students:
            continue
        new_row = dict(row)
        new_row["missing_students"] = failed_students
        new_row["missing_count"] = len(failed_students)
        updated_rows.append(new_row)

    summary = dict(st.session_state.get("clases_inclusiva_alumnos_summary") or {})
    summary["clases_con_pendientes"] = len(updated_rows)
    summary["alumnos_faltantes"] = sum(
        int(row.get("missing_count") or 0)
        for row in updated_rows
        if isinstance(row, dict)
    )
    st.session_state["clases_inclusiva_alumnos_rows"] = updated_rows
    st.session_state["clases_inclusiva_alumnos_summary"] = summary
...
def _render_clases_inclusiva_alumnos_assignment_section(
    colegio_id_raw: str,
    token: str,
    empresa_id: int,
    ciclo_id: int,
    ...
    def _run_apply(selected_rows: Sequence[Dict[str, object]]) -> None:
        apply_bar = st.progress(0)
        apply_status = st.empty()

        def _apply_progress(current: int, total: int, message: str) -> None:
            apply_bar.progress(
                min(1.0, max(0.0, current / max(total, 1))), text=message
            )
            apply_status.caption(message)

        with st.spinner("Asignando alumnos faltantes..."):
            apply_summary, result_rows = _apply_santillana_inclusiva_students_assignment(
                token=token,
                rows=selected_rows,
                empresa_id=int(empresa_id),
                ciclo_id=int(ciclo_id),
        ...
        apply_bar.progress(1.0, text="Asignacion terminada.")
        apply_status.caption("Asignacion terminada.")
        ...
        for res in result_rows:
            if not isinstance(res, dict):
                continue
            cid_res = _safe_int(res.get("Clase ID"))
            if cid_res is None:
                continue
            applied_ids.add(int(cid_res))
            failed_by_class[int(cid_res)] = [
                s for s in (res.get("_failed_students") or []) if isinstance(s, dict)
            ]
        updated_pc = dict(
            st.session_state.get("clases_inclusiva_alumnos_per_colegio") or {}
        )
        for cid_k, cdata in updated_pc.items():
            if not isinstance(cdata, dict):
                continue
            new_rows: List[Dict[str, object]] = []
            for row in (cdata.get("rows") or []):
                if not isinstance(row, dict):
                    continue
                clase_id_r = _safe_int(row.get("clase_id"))
                if clase_id_r is None or int(clase_id_r) not in applied_ids:
                    new_rows.append(row)
                    continue
                failed = failed_by_class.get(int(clase_id_r), [])
                if failed:
                    nr = dict(row)
                    nr["missing_students"] = failed
                    nr["missing_count"] = len(failed)
                    new_rows.append(nr)
    ...
    if pending_rows:
        ...
        st.download_button(
            ...
            file_name="pendientes_santillana_inclusiva.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key="clases_inclusiva_alumnos_pending_download",
            use_container_width=True,
        )
    ...
    if results_cached:
        ...
        with st.container(border=True):
            st.markdown("**Ultimo resultado de asignacion**")
            _show_dataframe(visible_results, use_container_width=True)
            st.download_button(
                "Descargar resultado",
                data=_export_simple_excel(
                    visible_results,
                    sheet_name="resultado_si",
                ),
                file_name="resultado_asignacion_santillana_inclusiva.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                key="clases_inclusiva_alumnos_result_download",
                use_container_width=True,
            )
...
def _render_santillana_inclusiva_student_removal(
    token: str,
    empresa_id: int,
    ciclo_id: int,
    timeout: int,
    ...
    if run_preview:
        if not token:
            ...
            colegio_rows = [
                ...
                for row in (st.session_state.get("shared_colegios_rows") or [])
                if isinstance(row, dict)
                and _safe_int(row.get("colegio_id")) is not None
            ]
            ...
            if colegio_catalog_error:
                ...
                with st.spinner("Buscando clases Santillana Inclusiva..."):
                    for idx, colegio_row in enumerate(colegio_rows, start=1):
                        ...
                        progress_bar.progress(
                            min(
                                1.0,
                                max(0.0, (idx - 1) / max(total_colegios, 1)),
                            )
                        )
                        status_box.caption(
                        ...
                        for clase in clases:
                            if not isinstance(clase, dict):
                                continue
                            preview_row = (
                                _build_santillana_inclusiva_removal_class_row(
                                    clase,
                                    colegio_label=colegio_label,
                                    colegio_id=int(colegio_id),
                                )
                            ...
                            if not isinstance(preview_row, dict):
                                continue
                            clase_id = _safe_int(preview_row.get("_clase_id"))
                            class_identity = (
                                int(colegio_id),
                                int(clase_id or 0),
                            )
                ...
                preview_rows.sort(
                    key=lambda row: (
                        ...
                        _participantes_grado_sort_rank(row.get("Grado")),
                        str(row.get("Nombre curso") or "").upper(),
                        int(_safe_int(row.get("_clase_id")) or 0),
                    )
                )
                st.session_state["reportes_retiro_si_preview_rows"] = preview_rows
                st.session_state["reportes_retiro_si_preview_errors"] = preview_errors
                st.session_state["reportes_retiro_si_scope"] = {
                    "colegios_total": int(total_colegios),
                    "colegios_error": len(preview_errors),
                    "empresa_id": int(empresa_id),
                    "ciclo_id": int(ciclo_id),
                ...
                st.session_state["reportes_retiro_si_widget_version"] = (
                    int(
                        st.session_state.get(
                            "reportes_retiro_si_widget_version",
                            0,
                        )
                        or 0
                    )
                ...
                st.session_state.pop("reportes_retiro_si_result_rows", None)
    ...
    if isinstance(preview_rows_cached, list):
        preview_rows_cached = [
            row for row in preview_rows_cached if isinstance(row, dict)
        ]
        preview_errors_cached = [
            str(item).strip()
            for item in (
                st.session_state.get("reportes_retiro_si_preview_errors") or []
            )
            if str(item).strip()
        ]
        preview_scope = st.session_state.get("reportes_retiro_si_scope") or {}
        preview_matches_context = bool(token) and (
            str(preview_scope.get("token_fingerprint") or "")
            == hashlib.sha256(token.encode("utf-8")).hexdigest()
            and _safe_int(preview_scope.get("empresa_id")) == int(empresa_id)
            and _safe_int(preview_scope.get("ciclo_id")) == int(ciclo_id)
        )
        if not preview_rows_cached:
            st.info(
                "No se encontraron cursos cuyo nombre comience con "
            ...
            professor_count = sum(
                len(row.get("Profesores") or []) for row in selected_rows
            )
            ...
            if selected_rows and participants_loaded:
                ...
                with st.expander(
                    f"Alumnos que se retiraran ({len(selected_student_rows)})",
                    expanded=False,
                ):
                    if selected_student_rows:
                        _show_dataframe(
                            selected_student_rows,
                            use_container_width=True,
                        )
                        st.download_button(
                            "Descargar previsualizacion de alumnos",
                            data=_export_simple_excel(
                                selected_student_rows,
                                sheet_name="alumnos_a_retirar",
                            ),
                            file_name="previsualizacion_retiro_santillana_inclusiva.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                ...
                st.warning(
                    "Esta accion retirara solamente los alumnos listados arriba. "
                ...
                run_removal = st.button(
                    ...
                    disabled=(
                        ...
                        or not preview_matches_context
                    ),
                    use_container_width=True,
                )
                if run_removal:
                    if not token:
                        ...
                        st.error(
                            "La previsualizacion ya no corresponde al token o ciclo actual."
                        )
                    elif not confirm_removal:
                        st.error("Confirma la accion antes de retirar alumnos.")
                    else:
                        progress_bar = st.progress(0)
                        status_box = st.empty()
                        result_rows: List[Dict[str, object]] = []
                        removed_pairs: Set[Tuple[str, int]] = set()
                        total_students = len(selected_student_rows)
                        processed = 0
                        for class_row in selected_rows:
                            class_key = str(class_row.get("_key") or "")
                            clase_id = _safe_int(class_row.get("_clase_id"))
                            if clase_id is None:
                                continue
                            for alumno in class_row.get("Alumnos") or []:
                                if not isinstance(alumno, dict):
                                    continue
                                alumno_id = _safe_int(alumno.get("alumno_id"))
                                if alumno_id is None:
                                    continue
                                processed += 1
                                progress_bar.progress(
                                ...
                                status_box.caption(
                                    "[{idx}/{total}] {alumno} | {curso}".format(
                                        ...
                                        curso=str(
                                            class_row.get("Nombre curso") or "Curso"
                                        ),
                                    )
                                )
                                result_row = {
                                    "Colegio": class_row.get("Colegio", ""),
                                    "Clase ID": int(clase_id),
                                    "Nombre curso": class_row.get(
                                        "Nombre curso",
                                        "",
                                    ),
                                    "Alumno ID": int(alumno_id),
                                    "Alumno": alumno.get("nombre_completo", ""),
                                    "Login": alumno.get("login", ""),
                                    "Estado": "",
                                    "Detalle": "",
                                ...
                                try:
                                    ...
                                    result_row["Estado"] = "Retirado"
                                    removed_pairs.add((class_key, int(alumno_id)))
                                except Exception as exc:  # pragma: no cover - UI
                                    result_row["Estado"] = "Error"
                                    result_row["Detalle"] = (
                                        str(exc).strip() or "error sin detalle"
                                    )
                        ...
                        ok_count = sum(
                            1
                            for row in result_rows
                            if row.get("Estado") == "Retirado"
                        )
                        error_count = len(result_rows) - ok_count
                        status_box.caption(
                            "Retiro terminado: {ok} alumno(s) retirado(s), "
                            "{errors} error(es).".format(
                                ok=ok_count,
                                errors=error_count,
                            )
                        )
                        ...
                        for class_row in preview_rows_cached:
                            ...
                            updated_row["Alumnos"] = [
                                ...
                                for alumno in (class_row.get("Alumnos") or [])
                                if not (
                                    isinstance(alumno, dict)
                                    and _safe_int(alumno.get("alumno_id")) is not None
                                    and (
                                        class_key,
                                        int(_safe_int(alumno.get("alumno_id")) or 0),
                                    )
                                    in removed_pairs
                                )
                            ]
        ...
        if preview_errors_cached:
            with st.expander(
                ...
                st.write(
                    "\n".join(f"- {item}" for item in preview_errors_cached[:120])
                )
                pending = len(preview_errors_cached) - 120
                if pending > 0:
                    st.caption(f"... y {pending} errores mas.")

    result_rows_cached = st.session_state.get("reportes_retiro_si_result_rows")
    if isinstance(result_rows_cached, list):
        result_summary = (
            st.session_state.get("reportes_retiro_si_result_summary") or {}
        )
        retirados = int(result_summary.get("retirados") or 0)
        errores = int(result_summary.get("errores") or 0)
        if errores:
            st.warning(
                f"Ultimo retiro: {retirados} alumno(s) retirado(s), {errores} error(es)."
            )
        else:
            st.success(f"Ultimo retiro completado: {retirados} alumno(s) retirado(s).")
        if result_rows_cached:
            st.download_button(
                "Descargar resultado del retiro",
                data=_export_simple_excel(
                    result_rows_cached,
                    sheet_name="resultado_retiro",
                ),
                file_name="resultado_retiro_santillana_inclusiva.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                key="reportes_retiro_si_result_download",
                use_container_width=True,
            )
...
def _render_otras_funcionalidades_view() -> None:
    ...
    with st.container(border=True):
        st.markdown("**Excel masivo estudiantes**")
        st.caption(
        ...
        run_excel_masivo_estudiantes = st.button(
            ...
            type="primary",
            key="otras_excel_masivo_estudiantes_btn",
            use_container_width=True,
            disabled=not bool(row_by_id_multi),
        )
        if not row_by_id_multi:
            st.caption("Guarda un token para cargar la lista completa de colegios.")

    if run_excel_masivo_estudiantes:
        if not token:
            st.error("Falta el token. Configura el token global o PEGASUS_TOKEN.")
            st.stop()

        colegio_ids_excel_masivo = sorted(
            row_by_id_multi.keys(),
            key=lambda value: str(
                (row_by_id_multi.get(int(value)) or {}).get("colegio") or ""
            ).upper(),
        )
        if not colegio_ids_excel_masivo:
            st.error("No hay colegios disponibles para procesar.")
            st.stop()
    ...
    excel_masivo_summary = (
        st.session_state.get("otras_excel_masivo_estudiantes_summary") or []
    )
    excel_masivo_errors = (
    ...
    if excel_masivo_summary:
        with st.container(border=True):
            result_cols = st.columns([2.2, 1, 1, 1.5], gap="small")
            ...
            _show_dataframe(excel_masivo_summary, use_container_width=True)
    if excel_masivo_errors:
        with st.expander(
            f"Errores de Excel masivo estudiantes ({len(excel_masivo_errors)})",
            expanded=False,
        ):
            st.markdown(
                "\n".join(f"- {item}" for item in excel_masivo_errors[:100])
            )
            pending_errors = len(excel_masivo_errors) - 100
            if pending_errors > 0:
